...
-- Explicação: O nome lerp que representa esta função é utilizado em diversas partes do código e é associado a sua respectiva implementação em tempo de compilação.
    ...
    elseif gamemode == 1 then
        if gameover == false then
            ...
            for key,value in pairs(entities) do
                ...
                -- Explicação: A chamada a função update() refere-se a um campo update localizado em uma tabela qualquer. A implementação dessa função pode variar dependendo de qual tabela está sendo utilizada naquele momento, e só é determinada em tempo de execução.
            end
        end
    end
end
        ...
        if gameover == true then
            ...
            if menu.vaipromenu == nil then
                ...
                function(self)
                    ...
                    -- Explicação: A decisão de que o operador # obtém o tamanho de um vetor foi feita no momento do design da linguagem.
...
function love.keypressed(key)
	...
	-- Propriedade: Valor
	-- Binding Time: Execução
	-- Explicação: A variável key refere-se a entrada do usuário que está utilizando o programa, portanto o valor da variável key só é determinado em tempo de execução.
    if gamemode == 1 then
        if key == "left" then
            leftpressed = true
...
function initializeGame()
    ...
    function(self)
        ...
        -- Tarefa-05
        -- Nome: Função math.floor()
        -- Propriedade: Semântica
        -- Binding Time: Design da linguagem
        -- Explicação: A função math.floor() é uma função que já vem na biblioteca padrão da linguagem lua e portanto seu nome e implementação foi definida no tempo de design da linguagem.
    end,
    function(self,dt)
...
function initializePlayerEntity()
    ...
    player.OutOfBoundsY = function(newy)
        if newy <= 64*2.5 or newy + player.height >= love.graphics.getHeight()  then
        	...
        	-- Propriedade: Semântica
        	-- Binding Time: Compilação
        	-- Explicação: A semântica do operador * varia dependendo dos tipos dos seus dois operandos. Nesse caso, por exemplo, a operação 64*2.5 tem como seu resultado um valor de ponto flutuante pois um de seus operandos também é de ponto flutuante. Essa semântica do operador * (quando ambos os valores são constantes) é definida em tempo de compilação.
            return true
        end
        return false
    end
